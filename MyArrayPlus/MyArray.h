...
class MyArray {
 ...
 public:
  ...
  MyArray();
  MyArray(const MyArray& temp);

  MyArray(T* ip, int size);
  ~MyArray();
  MyArray operator+(const MyArray& p1);
  // void operator=(const MyArray& p1);
  // friend ostream& operator<<(ostream& out, const MyArray& p1);
};
#include "MyArray.h"
template <class T>
